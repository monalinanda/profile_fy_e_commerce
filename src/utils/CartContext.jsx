...
export const CartContext = ({ children }) => {
    ...
    try {
      ...
      if (existingItem) {
        ...
        const response = await fetch(
          `https://profile-fyi-backend.vercel.app/cart/${product.id}`,
          {
            method: "PUT",
  ...
  const removeFromCart = async (productId) => {
    try {
      const response = await fetch(`https://profile-fyi-backend.vercel.app/cart/${productId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
  ...
  const updateQuantity = async (productId, newQuantity) => {
    try {
      ...
      if (item) {
        ...
        const response = await fetch(
          `https://profile-fyi-backend.vercel.app/cart/${productId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(updatedItem),
